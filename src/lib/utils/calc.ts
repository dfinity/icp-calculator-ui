import { calculators, Direction, Duration, Mode, type Bytes, type Cycles, type Instructions, type USD } from "@dfinity/icp-calculator";
...
export function execution(mode: Mode, instructions: Instructions, count: number): Amount {
...
export function storage(size: Bytes, duration: Duration, count: number): Amount {
    let usd = calcUSD.storage(size, duration) * count as USD;
    let cycles = calcCycles.storage(size, duration) * count as Cycles;
    return { usd, cycles };
}

export function message(mode: Mode, direction: Direction, bytes: Bytes, count: number): Amount {
...
export function httpOutcall(request: Bytes, response: Bytes, count: number): Amount {
