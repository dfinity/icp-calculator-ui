import { Call, Canister, Ingress, Storage, Timer, type Feature } from "./feature";
...
export function socialNetwork(users: number): Feature[] {
    ...
    storage.storage_index = storage.storage_values.findIndex(x => x >= users * BYTES_PER_USER);
    features.push(storage);

    let ingress = new Ingress();
    ingress.instruction_index = ingress.instruction_values.findIndex(x => x >= INSTRUCTIONS_PER_INGRESS);
    ingress.count = users * INGRESS_PER_USER;
    features.push(ingress);

    let timer = new Timer();
    timer.instruction_index = timer.instruction_values.findIndex(x => x >= INSTRUCTIONS_PER_TIMER);
    features.push(timer);
...
export function decentralizedExchange(tradesPerDay: number): Feature[] {
    ...
    let storage = new Storage();
    storage.storage_index = storage.storage_values.findIndex(x => x >= tradesPerDay * STORAGE_BYTES_PER_TRADE * STORAGE_HISTORY_DAYS);
    features.push(storage);
