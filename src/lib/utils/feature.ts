import { type Bytes, Duration, type Instructions, Mode, Direction } from "@dfinity/icp-calculator";
...
export class Storage implements Feature {
    ...
    cost(): Breakdown {
        ...
        result.add(new Cost(Kind.PerDay, Category.Storage, storage(bytes, Duration.fromDays(1), this.count)));
        return result;
    }
}
...
export class Ingress implements Feature {
    ...
    cost(): Breakdown {
        let result = new Breakdown();
        let instructions = this.instruction_values[this.instruction_index] as Instructions;
        let network = (this.request_values[this.request_index] + this.response_values[this.response_index]) as Bytes;
        ...
        result.add(new Cost(kind, Category.ExecutionIngress, execution(Mode.Replicated, instructions, count)));
        result.add(new Cost(kind, Category.NetworkIngress, message(Mode.Replicated, Direction.UserToCanister, network, count)));
        return result;
    }
}
...
export class Call implements Feature {
    ...
    cost(): Breakdown {
        ...
        let count = repeat == 0 ? this.count : this.count * repeat;
        let kind = repeat == 0 ? Kind.OneTime : Kind.PerDay;
        result.add(new Cost(kind, Category.ExecutionCall, execution(Mode.Replicated, instructions, count)));
        result.add(new Cost(kind, Category.NetworkCall, message(Mode.Replicated, Direction.CanisterToCanister, network, count)));
        return result;
    }
}
...
export class Timer implements Feature {
    ...
    cost(): Breakdown {
        ...
        let count = repeat == 0 ? this.count : this.count * repeat;
        let kind = repeat == 0 ? Kind.OneTime : Kind.PerDay;
        result.add(new Cost(kind, Category.ExecutionTimer, execution(Mode.Replicated, instructions, count)));
        return result;
    }
}
...
export class Heartbeat implements Feature {
    ...
    cost(): Breakdown {
        ...
        let instructions = this.instruction_values[this.instruction_index] as Instructions;
        let count = 24 * 3600;
        result.add(new Cost(Kind.PerDay, Category.ExecutionHeartbeat, execution(Mode.Replicated, instructions, count)));
        return result;
    }
}
export class HttpOutcall implements Feature {
    ...
    cost(): Breakdown {
        ...
        let count = repeat == 0 ? this.count : this.count * repeat;
        let kind = repeat == 0 ? Kind.OneTime : Kind.PerDay;
        result.add(new Cost(Kind.PerDay, Category.HttpOutcall, httpOutcall(request, response, count)));
        return result;
    }
}
